...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ user }) => {
  ...
  const handleMediaSubmit = (formData: any) => { if (editingMedia) store.updateMedia(editingMedia.id, formData); else store.addMedia({ ...formData, userId: user.id }); fetchData(); closeMediaModal(); };
  ...
  const handleDeleteMedia = (id: string) => { if (window.confirm('Are you sure?')) { store.deleteMedia(id); fetchData(); } };
